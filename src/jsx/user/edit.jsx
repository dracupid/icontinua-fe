...
class Edit extends React.Component {
  ...
  render () {
    ...
    return <div>
      <Banner title="我的资料" backUrl={util.getUrlByHash(`/${this.props.params.userId}`)}
      ...
      <div className='block-wrapper' style={{marginTop: 0}}>
        ...
        <EditBlock tag='用户名' type="text" value={nickname} editing={this.state.editing}
                   onChange={this.onChangeBuilder('nickname')}/>
        <EditBlock tag='性别' type="radio" data={{'1': '男', '2': '女'}} defaultValue={sex}
                   value={util.parseSex(sex)} editing={this.state.editing} onChange={this.onChangeBuilder('sex')}/>
        <EditBlock tag='年龄' type="number" min={1} max={130}
