...
export default function (props) {
  ...
  if (props.totalRent) {
    content = <div className="total-info">
      <span>合计 <span
        className="value">{'¥ ' + (props.totalDeposit + props.totalRent).toFixed(1)}</span></span>
      &nbsp;(<span>租金 <span className="value">{'¥ ' + props.totalRent.toFixed(1)}</span></span>&nbsp;+&nbsp;
      <span>押金 <span className="value">{'¥ ' + props.totalDeposit.toFixed(1)}</span></span>)
    </div>
  }
  else {
    content = <div className="total-info">
